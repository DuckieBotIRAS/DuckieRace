...
# Fix ROS GPG key - remove all existing ROS repos, then recreate with trusted flag
RUN rm -f /etc/apt/sources.list.d/ros*.list
RUN find /etc/apt/sources.list.d/ -name "*ros*" -delete || true
RUN grep -v "packages.ros.org" /etc/apt/sources.list > /tmp/sources.list && mv /tmp/sources.list /etc/apt/sources.list || true
